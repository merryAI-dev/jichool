# MYSC 그룹웨어 도우미 (지출결의·연차)

카드값 지출결의와 연차·휴가를 Claude와 대화하면서 처리합니다.

**지출결의는** 현대카드 명세서 PDF를 주면 그룹웨어에서 본인 카드 내역을 불러와 한 건씩 대조하고, 적요·용도·프로젝트를 물어본 뒤 전자결재에 **임시보관**까지 해둡니다.

**연차·휴가는** 남은 연차와 사용 내역을 확인하고, 신규 휴가의 날짜와 차감 일수를 그룹웨어 값으로 계산해 임시저장합니다. 상신이 확인된 휴가는 공유 캘린더에 등록할 수 있습니다.

**결재를 대신 올리지는 않습니다.** 임시보관까지만 하고 멈추므로, 마지막에 본인이 그룹웨어에서 확인하고 직접 상신하시면 됩니다.

## 설치 (개발자가 아니어도 됩니다)

**1. Claude Code가 설치되어 있어야 합니다.** 아직 없다면 AXR팀에 문의해주세요.

**2. 터미널을 엽니다.** `command`+`스페이스`를 누르고 `터미널`이라고 입력한 뒤 엔터를 누르면 열립니다.

**3. 아래 한 줄을 복사해서 터미널에 붙여넣고 엔터를 누릅니다.**

```sh
git clone https://github.com/merryAI-dev/jichool.git ~/.claude/skills/jichool
```

**4. Claude Code를 새로 시작하고 이렇게 말하면 됩니다.**

> 지출결의 해줘

> 연차 얼마나 남았어?

끝입니다. 나머지 준비(PDF 도구 설치, 그룹웨어 연결, 카드 프로필)는 **Claude가 한 단계씩 안내**합니다. 이 문서 아래쪽 명령들을 직접 외우거나 입력할 필요는 없습니다.

막히면 AXR팀에 문의해주세요. 업데이트는 `cd ~/.claude/skills/jichool && git pull` 입니다.
...
Claude가 따르는 진행 절차는 같은 폴더의 `SKILL.md`에 있으며, 스킬은 아래 명령과 JSON 응답을 호출할 뿐 새로운 API 연동을 추가하지 않습니다.
...
- 완전일치 이력 조회는 사업자등록번호(`regNb`)가 없는 행을 건너뜁니다. 해외 결제(AWS·Fly.io 등)는 사업자번호가 없어 **실제 승인 이력이 있어도 0건으로 나옵니다.** 0건을 기록 없음으로 단정하지 말고 `--query`로 다시 찾으십시오.
...
`--query`는 사업자등록번호·가맹점명 완전일치를 건너뛰고 가맹점명과 적요를 부분검색합니다. 회사·카드 일치 조건은 그대로 적용됩니다. 실측으로 `Amazon web services`는 0건에서 14건, `FLY.IO`는 0건에서 3건으로 복구됐습니다.
...
2026-09-08 실서버에서 PDF 없는 경로를 한 사이클 검증했습니다. `preview`로 8건 961,901원을 수집해 `dates`·`edit`·`check`를 거쳐 임시보관까지 성공했습니다. 이때 확인된 사항은 다음과 같습니다.

- `preview`는 PDF 없이 `check`까지 통과하지만 `statementCount`/`statementTotal`이 비어 있어 **누락 검증이 없습니다.** 외화·부분취소(`amount_mismatch`)도 PDF 대조에서만 잡히므로, 실제 상신용 초안은 `prepare` 경로를 사용하십시오.
- 용도(`cashCd`)를 입력하면 그 코드에 따라 `프로젝트 필수`가 새로 생길 수 있습니다. 필수 항목은 용도에 종속적입니다.
- 카드 승인 전표에는 결제 시각(`formatedIssDtTime`)만 있고 출발·도착 시각은 없습니다. `takeDt`/`returnDt`는 카드 결제에서 채워지지 않습니다. 야근 택시비의 출발 시각은 사용자에게 확인해야 합니다.
- 서버 응답을 그대로 보관하므로 검토안 JSON의 `source.receipts[].cardNb`에 카드번호 원본이 들어갑니다. 검토안·검토 화면 파일은 `0600`으로 생성되며 검토 화면 HTML에는 카드번호가 포함되지 않습니다.
...
[휴가 스킬](.claude/skills/leave-calendar/SKILL.md)을 이 저장소와 함께 사용합니다.
Claude가 현재 세션에 연결된 캘린더 도구로 검색·생성·재조회를 수행하고,
`calendar_harness.py`가 조회한 일정의 날짜·제목·참석자·문서 표식을 대조합니다.
별도 OAuth 클라이언트나 다른 LLM 프로세스를 실행하지 않습니다.

사용자가 임시저장과 캘린더 등록을 함께 요청하면 두 결과가 확인되어야 완료입니다.
단계별 명령과 도구 응답 형식은 [Harness 실행 지침](.claude/skills/leave-calendar/references/harness.md)에 있습니다.
로컬 작업 기록으로 응답 유실 후 무조건 재생성을 막습니다. 같은 기록을 사용하는
여러 세션의 동시 실행은 지원하지 않습니다.

```sh
python3 test_calendar_harness.py
python3 test_calendar_harness.py /비공개/일정요청.json
```

Harness 검증은 도구 응답 재현 테스트입니다. 실제 캘린더 생성·초대 전송을 검증한 것은
아닙니다. 실행하는 Claude 환경에 대상 공유 캘린더의 쓰기 권한, 종일 일정 생성,
참석자 초대 및 재조회 도구가 있어야 합니다. 연결 도구가 시간 일정만 지원하면
종일 연차를 자정 시간 일정으로 대체하지 않고 미완료 상태로 남깁니다.
